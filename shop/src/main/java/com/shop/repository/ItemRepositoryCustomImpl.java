...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{
    ...
    private BooleanExpression regDtsAfter(String searchDateType){ //all, 1d, 1w, 1m 6m
        ...
        //dateTime을 시간에 맞게 세팅 후 시간에 맞는 등록된 상푸밍 조회하도록 조건값 반환
    }
    ...
    private BooleanExpression searchByLike(String searchBy, String searchQuery){
        ...
        }else if(StringUtils.equals("itemValue",searchBy)){
            return QItem.item.itemValue.stringValue().like(searchQuery+"%");
        }
        return null;
    }
    ...
    private BooleanExpression itemValueLike(String searchQuery) {
        return StringUtils.isEmpty(searchQuery) ? null : QItem.item.itemValue.stringValue().eq(searchQuery);
    }
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable){
        ...
        , itemImg.imgUrl, item.price))
                ...
                .where(itemNmLike(itemSearchDto.getSearchQuery())
                      ,itemValueLike(itemSearchDto.getSearchItemValue()))
